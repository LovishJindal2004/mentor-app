...
import { QuestionReponse } from '../test.model';
...
export class ExamGameReviewComponent implements OnInit {
  ...
  constructor(
    ...
    private courseExamService: TestService,
  ...
  loadQuestion() {
    var self = this;
    this.courseExamService.getpredefineExamAnswersheet(this.examid, this.courseId).subscribe((res: any) => {
  ...
  getquestiondetails(questionDetailID) {
    ...
    if (this.loadedQuestionDetails[questionDetailID]) {
      ...
      this.courseExamService.getQuestionbyID(questionDetailID,this.courseId).subscribe((response: any) => {
