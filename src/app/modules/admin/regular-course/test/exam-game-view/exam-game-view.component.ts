...
import { parts, QuestionActivaty, QuestionReponse } from '../test.model';
...
export class ExamGameViewComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private _examService: TestService, private _ngZone: NgZone, private router: Router,
  ...
  loadQuestion() {
    var self = this;
    this._examService.getsigleExam(this.courseId, this.ExamId).subscribe((res: any) => {
  ...
  viewresult() {
    this.dialog.closeAll();
    this._ngZone.run(() => {
      this.router.navigate([`/exam-management/game-analytics`, this.ExamId]);
    });
  }
  //Start timer
  startTimer() {
