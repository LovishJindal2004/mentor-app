...
export class ExamGameAnalyticsComponent implements OnInit {
  ...
  constructor(
    ...
    this.courseId = this._dataGuard.getCourseId();
  ...
  ngOnInit(): void {
    ...
    if(this.isCBTExamType != undefined){
      this._examService.getpredefineExamResultv2(this.examId,this.courseId).subscribe(res=>{
      ...
      this._examService.getpredefineExamResult(this.examId,this.courseId).subscribe(res=>{
        this.examDetails = res.examDetails;
        this.examActivity = res.activity[0];
        this.examSubjectsWiseAnalytics = res.subjectsWiseAnalytics;
        this.dataLoaded = true;
  ...
  getTopppers(){
    this._examService.getExamTopScorers(this.examId,this.courseId,this.toppersByState).subscribe(res=>{
      this.topScorers = res;
    });
  }
