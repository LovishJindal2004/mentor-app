...
import { QuestionActivaty, QuestionReponse } from '../custom-qbank.model';
...
export class GameViewComponent  implements OnInit {
      ...
      if (response) {
        if (this.isuserleftDirect) {
          ...
          if (!isQuestionComplete) {
            this._ngZone.run(() => {
              this.router.navigate(['/CustomQbank/list/']);
            });
          }
          else {
  ...
  viewresult() {
    this.dialog.closeAll();
    this._ngZone.run(() => {
      this.router.navigate(['/CustomQbank/game-analytics', this.ExamId]);
    });
  }
  calcluteCount() {
    var self = this;
