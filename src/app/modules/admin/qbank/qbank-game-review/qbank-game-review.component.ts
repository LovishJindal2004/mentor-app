...
import { QuestionReponse } from '../qbank.model';
...
export class QbankGameReviewComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private _qbnakservice: QBankService, private errorhandling: ApiErrorHandlerService,
  ...
  loadQuestion() {
    var self = this;
    this._qbnakservice.getQbnkAnswersheet(this.taskGuid, this.ExamId).subscribe((res: any) => {
      if(res){
  ...
  getquestiondetails(questionDetailID) {
    ...
    if (this.loadedQuestionDetails[questionDetailID]) {
      ...
      this._qbnakservice.getQbnkquestionDetailById(questionDetailID,this.ExamId, this.taskGuid).subscribe((response: any) => {
