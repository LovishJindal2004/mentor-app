...
import { QuestionActivaty, QuestionReponse } from '../qbank.model';
...
export class QbankGameViewComponent implements OnInit {
  ...
  loadQuestion() {
    var self = this;
    this._qbankservice.getQbankExamQuestion(this.taskGuid, this.ExamId,this.isQuestionTimeExist).subscribe((res: any) => {
      if(res){
  ...
  viewresult() {
    this.dialog.closeAll();
    this._ngZone.run(() => {
      this.router.navigate(['/qbank/game-analytics', this.ExamId, this.taskGuid]);
    });
  }
  calcluteCount() {
    var self = this;
  ...
  getQuestionDetail(QuestionDetailId) {
    ...
    this.IsExplanationView = false;
    this.ChoicesLoaded = false;
    this._qbankservice.getQbnkquestionDetailById(QuestionDetailId, this.ExamId, this.taskGuid).subscribe(res => {
      if (res) {
